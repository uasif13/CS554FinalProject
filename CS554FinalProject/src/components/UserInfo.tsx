...
const UserInfo  = () => {
		...
		async function fetchData(){
			if (currentUser){
				...
				await retrieveUserData(currentUser.email).then((data) => {
					...
					data.phoneNumber == "1234567890" ? setPhoneNumber("") : setPhoneNumber(data.phoneNumber);
				});
			}
		}
	...
	return(
		<div>
			<br/>
			<Header doesGoToProfile={false} doesGoToScheduler={true} doesSignOut={true}doesEdit ={true}/>
			<h1>Hi {name}</h1>
